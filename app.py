...
def submit_application():
    ...
    if selected_napbox_info:
        try:
            ...
            # ✅ I-VERIFY na existing talaga ang napbox_id sa DB bago gamitin
            if candidate_id:
    ...
    if is_reapply and original_application_id:
        ...
        try:
            ...
            print(f"❌ CRITICAL: Failed to UPDATE application {application_number}")
        ...
        try:
            ...
            if result is None:
                raise Exception("execute_query returned None — insert likely failed (check DB constraint errors above)")
            print(f"✅ New application {application_number} saved with preferred NAP Box")
        except Exception as e:
            print(f"❌ CRITICAL: Failed to INSERT application {application_number}")
            print(f"❌ Error type: {type(e).__name__}")
            print(f"❌ Error message: {e}")
            import traceback
            traceback.print_exc()
